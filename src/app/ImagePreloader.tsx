...
export default function HomeClient({ imagesToPreload }: { imagesToPreload: string[] }) {
  ...
  // simple per-file preload progress
  useEffect(() => {
    let cancelled = false;
    setLoadedCount(0);
    const handlers: HTMLImageElement[] = [];
    ...
    imagesToPreload.forEach((src) => {
      ...
      img.onload = done;
      img.onerror = done;
      // ensure we actually try to load local files (cache ok)
      img.src = src;
      handlers.push(img);
    });
  ...
  useEffect(() => {
    if (loadedCount === imagesToPreload.length && imagesToPreload.length > 0) {
      const t = setTimeout(() => setReady(true), 100);
      return () => clearTimeout(t);
    } else if (imagesToPreload.length === 0) {
      // no images: immediately ready
      setReady(true);
    }
  ...
  // client-only page logic (Lenis, GSAP, scroll triggers)
  useEffect(() => {
    if (!ready) return;
    ...
    const tick = (time: number) => lenis.raf(time * 1500);
  ...
  const pct = imagesToPreload.length === 0 ? 100 : Math.round((loadedCount / imagesToPreload.length) * 100);
  ...
  return (
    <>
      ...
      {/* Main content (kept hidden until animated in) */}
      <main ref={contentRef} className="opacity-0 w-full" style={{ willChange: "opacity, transform" }}>
        <ParallaxCardSection />
        <section className="top-0 sticky flex justify-center items-center bg-background px-6 h-screen">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h2
              className={`mb-8 ${fontGFSDidot.className} font-light text-primary text-4xl md:text-5xl text-balance leading-tight`}
            >
              Let&apos;s Create Something Beautiful Together
            </h2>
            <p className="mb-10 text-muted-foreground text-lg leading-relaxed">
              Every family has a unique story to tell. I would be honored to help you preserve your most precious
              moments with artistry and care that will be treasured for generations.
            </p>
            <Link
